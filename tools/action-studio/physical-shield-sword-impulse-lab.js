...
} from '../../src/combat/physical-shield-sword-impulse.js?v=g43b5r29';
...
const rim = new THREE.DirectionalLight(0x7fe2cf, 0.6); rim.position.set(-4, 3, 2); scene.add(rim);
...
shieldRim.position.z = -SHIELD_THICKNESS * 0.52;
...
const velocityArrow = new THREE.ArrowHelper(new THREE.Vector3(1, 0, 0), new THREE.Vector3(), 0.6, 0x66e7f4, 0.11, 0.055); velocityArrow.visible = false; scene.add(velocityArrow);
const impulseArrow = new THREE.ArrowHelper(new THREE.Vector3(1, 0, 0), new THREE.Vector3(), 0.6, 0xffa45f, 0.11, 0.055); impulseArrow.visible = false; scene.add(impulseArrow);
...
let shieldPreviousPosition = new THREE.Vector3();
let shieldPreviousQuaternion = new THREE.Quaternion();
let shieldPoseInitialized = false;
let tipWorld = new THREE.Vector3();
let comWorld = new THREE.Vector3();
let latestShieldContactSpeed = 0;
let latestTipSpeed = 0;
...
function setShieldPose(timeSeconds) {
  const motionTime = timeSeconds * speedScale();
  const p = smoothstep((motionTime - 0.018) / 0.205);
  shield.position.set(
    lerp(-0.25, 0.31, p),
    1.12 + Math.sin(p * Math.PI) * 0.025,
    lerp(0.025, -0.015, p),
  );
  shield.quaternion.setFromEuler(new THREE.Euler(
    Math.PI / 2 + deg(lerp(-2, 7, p)),
    deg(lerp(-24, 34, p)),
    deg(lerp(-8, 12, p)),
    'XYZ',
  ));
...
function resetSimulation() {
  ...
  hit = false;
  lastImpulse = null;
  previousSignedDistance = Infinity;
  shieldPoseInitialized = false;
  sword.position.set(0.12, 1.10, -1.10);
  ...
  latestTipSpeed = swordLinearVelocity.length();
  maxTipToComSpeedRatio = 1;
  setShieldPose(0);
  updateSwordMarkers();
  status.textContent = `${PHYSICAL_SHIELD_SWORD_IMPULSE_STAGE} READY · waiting for moving-shield contact`;
  status.className = 'warn';
  updateHud();
}

function shieldSurfaceAtTip() {
  const normal = new THREE.Vector3(0, 0, -1).applyQuaternion(shield.quaternion).normalize();
...
function applyPhysicalContact(surface) {
  ...
  const contactVelocityVector = new THREE.Vector3(shieldPointVelocity.x, shieldPointVelocity.y, shieldPointVelocity.z);
  if (contactVelocityVector.lengthSq() > 1e-8) {
    velocityArrow.position.copy(actualContact);
    velocityArrow.setDirection(contactVelocityVector.clone().normalize());
    velocityArrow.setLength(Math.min(1.15, 0.14 + contactVelocityVector.length() * 0.09), 0.11, 0.055);
    velocityArrow.visible = true;
  }
  const impulseVector = new THREE.Vector3(result.impulse.x, result.impulse.y, result.impulse.z);
  if (impulseVector.lengthSq() > 1e-8) {
    impulseArrow.position.copy(actualContact);
    impulseArrow.setDirection(impulseVector.clone().normalize());
    impulseArrow.setLength(Math.min(1.15, 0.16 + impulseVector.length() * 0.16), 0.11, 0.055);
    impulseArrow.visible = true;
  }
  hit = true;
  status.textContent = `${PHYSICAL_SHIELD_SWORD_IMPULSE_STAGE} CONTACT · impulse owns sword motion`;
...
function integrateDynamicSword(dt) {
  ...
  if (angularSpeed > 1e-6) {
    ...
    sword.quaternion.premultiply(dq).normalize();
  }
  ...
  latestTipSpeed = tipVelocity.length();
  const comSpeed = Math.max(0.001, swordLinearVelocity.length());
  maxTipToComSpeedRatio = Math.max(maxTipToComSpeedRatio, latestTipSpeed / comSpeed);
}
...
function fixedStep(dt) {
  ...
  if (simTime > 1.65) resetSimulation();
}

function updateHud() {
  hudPhase.textContent = `Phase: ${hit ? 'PHYSICAL REBOUND' : 'APPROACH / SHIELD SWEEP'} · t ${Math.round(simTime * 1000)}ms`;
...
function buildReport() {
  const report = {
    ...
    invariants: {
      noIKTarget: true,
      noPositionFollowRatio: true,
      shieldVelocityContributesAtContactPoint: true,
      impulseChangesSwordLinearVelocityImmediately: true,
      impulseChangesSwordAngularVelocityImmediately: true,
      bodyIntegrationDeferred: true,
    },
  };
...
function setPreset(kind) {
  if (kind === 'strong') {
    shieldSpeedInput.value = '1.75'; restitutionInput.value = '0.42'; frictionInput.value = '0.78'; contactBiasInput.value = '0.84';
  } else {
    shieldSpeedInput.value = '0.85'; restitutionInput.value = '0.18'; frictionInput.value = '0.42'; contactBiasInput.value = '0.68';
  }
...
function frame(timestamp) {
  ...
  accumulator += frameDt;
  let guard = 0;
  while (accumulator >= FIXED_DT && guard < 16) {
    fixedStep(FIXED_DT);
