...
if (!THREE?.WebGLRenderer || !THREE?.GLTFLoader) {
  throw new Error('G4.3A.3 requires Three.js + GLTFLoader');
}
...
const threatMarker = marker('G43A3_THREAT', 0x6cff86);
const bodyMarker = marker('G43A3_POST_BRACE', 0x4ca8ff);
const contactMarker = marker('G43A3_CONTACT', 0xff625f, 0.065);
...
function enterProductionGuard() {
  guardMachine.send(GUARD_EVENTS.RESET, { stage: ARTICULATED_IMPACT_BRACING_STAGE });
  guardRuntime.sync(camera);
  guardMachine.send(GUARD_EVENTS.GUARD_PRESS, { stage: ARTICULATED_IMPACT_BRACING_STAGE });
  guardRuntime.sync(camera);
...
function sampleAttacker(snapshot, deltaMs) {
  if (snapshot.action) {
    ...
    hudAttack.textContent = `Attack: ${profile.direction.toUpperCase()} · ${snapshot.phase} · ${time.toFixed(3)}s`;
    return;
  }

  if (attackerRecovery) {
    attackerRecovery.elapsedMs += deltaMs;
    const recovery = sampleLongswordAttackRecovery(
      attackerRecovery.direction,
      attackerRecovery.elapsedMs,
    );
...
function setBracingMode(mode) {
  bracingMode = mode;
  if (mode === 'off') bracingRuntime.resetImpact();
...
function updateDebug(postBraceSurface) {
  ...
  if (!visible) return;
  const point = latestBracePlan.analysis.threat.point;
  threatMarker.position.set(point.x, point.y, point.z);
  bodyMarker.position.set(
    postBraceSurface.center.x,
    postBraceSurface.center.y,
    postBraceSurface.center.z,
  );
}
...
function updateContact(snapshot, currentBlade, deltaSeconds) {
  ...
  if (latestContact.contact && !firstContact) {
    ...
    contactMarker.position.set(
      latestContact.point.x,
      latestContact.point.y,
      latestContact.point.z,
    );
    contactMarker.visible = true;
    bracingRuntime.triggerImpact({
      direction: snapshot.action.runtime.direction,
...
function updateHud() {
  ...
  const pose = latestBracing?.pose;
  const impact = latestBracing?.impact;
  ...
  hudBody.textContent = pose
    ? `Body: pelvis ${(pose.pelvisDropMeters * 100).toFixed(1)}cm · spine ${pose.spinePitchDegrees.toFixed(1)}° · knees L/R ${pose.leftKneeBendDegrees.toFixed(1)}°/${pose.rightKneeBendDegrees.toFixed(1)}°`
    : 'Body: —';

  hudImpact.textContent = impact
    ? `Impact: ${impact.phase} · scale ${impact.scale.toFixed(2)} · ${impact.direction || '—'}`
    : 'Impact: —';
...
function buildReport() {
  const report = {
    ...
    authority: 'presentation/probe only; G4.3A swept geometry remains contact authority and no PARRY_CONFIRMED is emitted',
    selectedDirection,
    bracingMode,
    latestBracePlan,
    latestBracing,
    ...
    invariants: {
      ...
      pelvisBaseDropMaxMeters: 0.022,
      feetRootTranslation: 'locked',
    },
    next: 'Visual review joint-chain compression and directional loading before G4.3B recoil.',
  };

  reportNode.textContent = JSON.stringify(report, null, 2);
  document.documentElement.dataset.g43a3 = report.pass ? 'pass' : 'fail';
  window.__G43A3_RESULT__ = report;
  return report;
}

async function main() {
  status.textContent = 'Loading UAL attacks + Skyrim Guard + Articulated Bracing…';
  const [ual1, ual2, skyrim] = await Promise.all([
    ...
    loadSkyrimConvertedAnimationLibrary(new THREE.GLTFLoader(), {
      THREE,
      rig: defender.rig,
      fps: 30,
    }),
  ...
  if (!bind?.correctionQuaternion) {
    throw new Error('G4.3A.3 requires Skyrim Guard weapon bind calibration');
  }
  ...
  status.textContent = report.pass
    ? 'G4.3A.3 PASS · articulated impact bracing ready'
    : 'G4.3A.3 FAIL';
...
document.querySelectorAll('[data-mode]').forEach((button) => {
  button.addEventListener('click', () => setBracingMode(button.dataset.mode));
});
...
main().catch((error) => {
  document.documentElement.dataset.g43a3 = 'fail';
  status.textContent = `G4.3A.3 FAIL · ${error?.message || error}`;
  status.className = 'bad';
  reportNode.textContent = error?.stack || String(error);
  window.__G43A3_RESULT__ = {
    stage: ARTICULATED_IMPACT_BRACING_STAGE,
    pass: false,
...
window.__G43A3_LAB__ = {
  ...
  get latestBracing() { return latestBracing; },
