...
test('historical G4.3B.5R.2.7 stays covered while R18I5 guarantees TOP/RIGHT deflect release', () => {
  ...
  assert.match(html, /latched DEFLECT_IMPULSE \+ the 7\/7 inspection \(all directions\), or confirmed-Parry fail-safe, starts a 28ms continuity bridge.*the weapon arm joins the running OLD B3/);
  assert.match(html, /LEFT drives the same full arm chain and releases normally/);
