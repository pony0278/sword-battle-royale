...
if (!browser) throw new Error('R18N.3 v6.1 probe requires BROWSER');
const debugPort = Number(process.env.R18N3_V61_DEBUG_PORT || 9461);
const pageUrl = process.env.R18N3_V61_PAGE_URL || 'http://127.0.0.1:4175/tools/action-studio/shield-driven-contact-coupling-lab.html';
const profileDir = mkdtempSync(join(tmpdir(), 'r18n3-v61-chrome-'));
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const targetTtcs = (process.env.R18N3_V61_TTCS || '0.150,0.130,0.110,0.090')
  .split(',').map(Number).filter((value) => Number.isFinite(value) && value > 0);
const trialsPerTarget = Number(process.env.R18N3_V61_TRIALS || 3);
...
async function pageTarget() {
  ...
  throw new Error('R18N.3 v6.1 probe could not attach to standalone lab');
}
...
async function waitFor(expression, timeoutMs = 45000) {
  ...
  throw new Error(`R18N.3 v6.1 timeout waiting for ${expression}`);
}
...
async function dispatchAtControlledTtc(targetTtc, trialIndex) {
  const restarted = await evaluate(`window.__G43B5R281_LAB__.restartAttack('right')`);
  if (restarted !== true) throw new Error(`R18N.3 v6.1 could not restart RIGHT trial ${trialIndex}`);
  ...
  const dispatch = await evaluate(`new Promise((resolve, reject) => {
    ...
    function tick() {
      ...
      if (!r || s?.direction !== 'right') {
        if (performance.now() > deadline) return reject(new Error('RIGHT attack snapshot unavailable'));
        return requestAnimationFrame(tick);
      }
      const elapsed = Number(s.elapsedSeconds);
      const contact = Number(r.contactSeconds);
      const movementStart = Number(r.movementStartSeconds);
      const ttc = contact - elapsed;
      if (elapsed >= movementStart && ttc > 0 && ttc <= targetTtc) {
        const result = lab.dispatchParryInput(${JSON.stringify(`r18n3-v61-controlled-ttc-${trialIndex}`)});
        return resolve({
          targetTtc,
          actualTtc: ttc,
          elapsedSeconds: elapsed,
          contactSeconds: contact,
          movementStartSeconds: movementStart,
          accepted: result?.accepted === true,
          result,
        });
      }
      if (ttc <= 0 || performance.now() > deadline) {
        return reject(new Error('missed controlled TTC crossing'));
      }
      requestAnimationFrame(tick);
    }
    requestAnimationFrame(tick);
  })`);
  if (dispatch?.accepted !== true) throw new Error(`R18N.3 v6.1 RIGHT F rejected at target TTC ${targetTtc}`);
  ...
  const final = await evaluate(`({ confirmation: window.__G43B5R281_LAB__.latestParryConfirmation, whiff: window.__G43B5R281_LAB__.latestParryWhiff })`);
  ...
  const convergence = Number.isFinite(firstIntentRemaining) && Number.isFinite(minFinalSurfaceRemaining)
    ? firstIntentRemaining - minFinalSurfaceRemaining
    : null;
  const targetDrift = targets.length ? Math.max(...targets.map((value) => distance(value, targets[0]))) : null;

  return {
    targetTtc,
    actualTtc: dispatch.actualTtc,
    elapsedSeconds: dispatch.elapsedSeconds,
    inputAccepted: dispatch.accepted,
    ...
    metrics: {
      ...
      finalSurfaceConvergenceMeters: convergence,
      targetDriftMeters: targetDrift,
...
try {
  const trials = [];
  let trialIndex = 0;
  for (const targetTtc of targetTtcs) {
    for (let repeat = 0; repeat < trialsPerTarget; repeat += 1) {
      trialIndex += 1;
      const trial = await dispatchAtControlledTtc(targetTtc, trialIndex);
      trials.push({ ...trial, repeat: repeat + 1 });
      console.log(`R18N3_V61_TRIAL=${JSON.stringify(trials.at(-1))}`);
      await sleep(80);
    }
  }
  const buckets = targetTtcs.map((targetTtc) => {
    const rows = trials.filter((trial) => trial.targetTtc === targetTtc);
    const actualTtcs = finite(rows.map((trial) => trial.actualTtc));
    return {
      targetTtc,
      trials: rows.length,
      confirmations: rows.filter((trial) => trial.confirmation).length,
      whiffs: rows.filter((trial) => trial.whiff).length,
      minActualTtc: actualTtcs.length ? Math.min(...actualTtcs) : null,
      maxActualTtc: actualTtcs.length ? Math.max(...actualTtcs) : null,
    };
  });
  const data = { stage: 'R18N.3-v6.1-right-contact-repro', targetTtcs, trialsPerTarget, trials, buckets };
  console.log(`R18N3_V61_PROBE_JSON=${JSON.stringify(data)}`);
} finally {
