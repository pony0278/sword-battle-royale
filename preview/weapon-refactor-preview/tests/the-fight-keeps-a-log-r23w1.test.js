...
test('R23W.1 the lab records the opponent\'s swing when it starts and settles it on its falling edge, stagger and all', () => {
  ...
  assert.match(entry, /if \(opponentWasSwinging && !snapshot\?\.action\) swingLedger\.settle\(\{ bodyHit: exchangeState\.latestBodyHit, outcome: exchangeState\.latestCombatResult\?\.resolution\?\.outcome, tier: exchangeState\.latestParryConfirmation\?\.tier, separationMeters: laneController\.settledSeparationMeters, receiverStaggered: attackerFighter\.condition\.report\.staggered \}\);/);
});
